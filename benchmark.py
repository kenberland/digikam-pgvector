...
# Import the conversion logic and data from our conversion script
from mysql_to_pgvector import convert_blob_to_vector, create_weight_bin, S_HAAR_WEIGHTS
...
def main():
    ...
    try:
        ...
        for i in range(num_runs):
            ...
            rand_cursor.execute(
                f"SELECT imageid, matrix FROM ImageHaarMatrix where imageid = 82411"
            )
            query_id, query_blob = rand_cursor.fetchone()
            query_vector = convert_blob_to_vector(query_blob, weights, weight_bin)
            print(f"Querying for image id: {query_id}")
            rand_cursor.close()

            # --- MySQL Benchmark (L2 Distance) ---
            print(
                "Running MySQL benchmark (client-side L2 distance)... ",
                end="",
                flush=True,
            )
            ...
            unbuffered_mysql_cursor.execute(
            ...
            # --- PostgreSQL Benchmark (pgvector L2 Distance) ---
            start_time = time.time()
            postgres_cursor.execute(
                "SELECT imageid, matrix <-> %s AS distance FROM ImageHaarMatrix ORDER BY distance LIMIT 20",
                (query_vector,),
            )
            ...
            # --- Verification ---
            mysql_top_10_ids = {res[0] for res in mysql_results[:20]}
